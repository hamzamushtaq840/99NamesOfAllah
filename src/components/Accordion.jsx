        ...
        >
          <div className={`${activeIndex.includes(index) ? '  ' : 'border-none '} p-4`}>
            <button className={`duration-350 mb-2 flex w-full justify-between transition-all`}>
              <h2
                className={`aref tracking-[0.05em] ${activeIndex.includes(index) ? 'text-lg text-white' : ' text-[#ccccccb1]'
                  } transition-all duration-500`}
              >
                {section.title}
              </h2>
              <span
                className={`${activeIndex.includes(index) ? 'rotate-[-90deg]' : 'rotate-[90deg]'
                  } transition-all duration-500`}
              >

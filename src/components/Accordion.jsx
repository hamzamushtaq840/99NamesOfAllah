      ...
      {accordions.map((section, index) => (
        ...
        ${activeIndex.includes(index) ? 'max-h-[1000px]' : 'max-h-[52px]'} overflow-hidden transition-all duration-500`}>
          <div className={`${activeIndex.includes(index) ? '  ' : 'border-none '} p-4`}>
            <button className={`w-full flex justify-between mb-2 transition-all duration-350`} >
              <h2 className={`aref tracking-[0.05em] ${activeIndex.includes(index) ? 'text-white text-lg' : ' text-[#ccccccb1]'} transition-all duration-500`}>{section.title}</h2>
